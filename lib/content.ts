...
export type SectionGroup = 'overview' | 'tutorials' | 'workflows' | 'advanced' | 'reference';
...
export const sections: Section[] = [
  {
    ...
    blocks: [
      ...
      },
      {
        type: 'links',
        items: [
          { label: 'AGENTS.md guide', href: 'https://developers.openai.com/codex/guides/agents-md', external: true },
          { label: 'Hooks guide', href: 'https://developers.openai.com/codex/hooks', external: true },
          { label: 'Rules guide', href: 'https://developers.openai.com/codex/rules', external: true },
          { label: 'Skills guide', href: 'https://developers.openai.com/codex/skills', external: true },
          { label: 'Subagents guide', href: 'https://developers.openai.com/codex/subagents', external: true },
          { label: 'Commands', href: 'https://developers.openai.com/codex/app/commands', external: true },
        ],
      },
    ],
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        tone: 'info',
        text:
          'The global layer should describe broad habits, not per-repo rituals. If it changes every week, it belongs in the repo. The global file is climate, not weather.',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        type: 'p',
        text:
          'Codex builds project instructions by layering the active instruction files from your Codex home and the repository tree. Put a focused AGENTS.md in the repo root, then add AGENTS.override.md deeper in the tree only where a narrower area needs to override the broader contract.',
      },
      {
        type: 'steps',
        items: [
      ...
      },
      {
        ...
        tone: 'warning',
        text:
          'Build Sequence is not a built-in Codex feature. The native way to express it is AGENTS.md plus, if needed, a UserPromptSubmit hook that reminds the agent to read and follow the sequence before implementation starts.',
      },
    ],
  },
  {
  ...
  "hooks": {
    "UserPromptSubmit": [
      ...
      },
      {
        type: 'p',
        text:
          'That hook should not invent its own process. Its job is simple: read the active AGENTS.md, detect whether a Build Sequence exists, and remind Codex to follow the first unchecked step before implementation begins.',
      },
    ],
  },
  {
...
5. Stop at the step boundary and summarize remaining work.`,
      },
      {
        ...
        tone: 'info',
        text:
          'Skills are a first-class Codex authoring format for reusable workflows. They are the native place to store a repeatable feature procedure without turning AGENTS.md into a phone book.',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        ...
        rows: [
          ...
          ['repo/path/AGENTS.override.md', 'Narrow override that replaces broader rules in that subtree'],
        ],
      },
      {
        type: 'list',
    ...
    blocks: [
      {
        type: 'p',
        text:
          'Codex discovers hooks.json next to active config layers and loads all matching files. The two most practical locations are ~/.codex/hooks.json and <repo>/.codex/hooks.json. UserPromptSubmit is the most useful event for process reminders. SessionStart and Stop are good for loading or saving context. PreToolUse and PostToolUse currently intercept Bash tool activity only.',
      },
      {
        type: 'table',
        ...
        rows: [
          ...
          ['UserPromptSubmit', 'Check prompt-time requirements such as a Build Sequence reminder'],
  ...
  "hooks": {
    "UserPromptSubmit": [
      ...
      },
      {
        ...
        tone: 'warning',
        text:
          'PreToolUse and PostToolUse currently only support Bash tool interception. They are guardrails, not a universal enforcement wall around every write or edit action.',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        type: 'p',
        text:
          'Rules live under .codex/rules or ~/.codex/rules and use Starlark. Use them to allow, prompt, or forbid command prefixes during escalations. Inline match examples act like tiny unit tests for your rule file.',
      },
      {
        type: 'code',
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        type: 'p',
        text:
          'Invoke skills explicitly with $ in the composer, and remember that enabled skills can also appear in the slash command list. Put the workflow in the skill, then keep the durable project contract in AGENTS.md.',
      },
      {
        type: 'code',
    ...
    blocks: [
      {
        type: 'p',
        text:
          'Codex ships with built-in agents including default, worker, and explorer. You can add custom agents as standalone TOML files under ~/.codex/agents or .codex/agents. Each custom agent must define name, description, and developer_instructions, and may also override normal config keys such as model, sandbox_mode, MCP servers, or skill config.',
      },
      {
        type: 'code',
    ...
    blocks: [
      {
        type: 'p',
        text:
          'In the Codex app, slash commands are built in and vary by environment and access. The documented built-ins include /feedback, /mcp, /plan-mode, /review, and /status. Skills can be invoked explicitly with $. Enabled skills may also appear in the slash command list.',
      },
      {
        type: 'table',
      ...
      },
      {
        ...
        tone: 'warning',
        text:
          'Custom slash commands such as /quick, /build, or /evolve are not presented here as native Codex commands. If you want those behaviors, model them with AGENTS.md, hooks, skills, or external tooling and label them honestly.',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        ...
        rows: [
          ...
          ['approval_policy', 'Control when Codex pauses before generated commands'],
          ['sandbox_mode', 'Control filesystem and network access during command execution'],
          ['project_doc_fallback_filenames', 'Treat existing filenames as instruction files'],
          ['project_doc_max_bytes', 'Raise instruction discovery limit if needed'],
    ...
    blocks: [
      {
        ...
        headers: ['Mode', 'Effect'],
        rows: [
          ['read-only + never', 'Read only, no approvals, useful for CI or analysis'],
          ['workspace-write + on-request', 'Edit in workspace, ask when needed, good general default'],
          ['workspace-write + untrusted', 'Edit freely but ask before untrusted commands'],
          ['dangerously-bypass-approvals-and-sandbox', 'No sandbox and no approvals, not recommended'],
        ],
      },
      {
        type: 'code',
      ...
      },
      {
        ...
        tone: 'info',
        text:
          'For a normal repository, workspace-write with on-request approvals is a practical default. Tighten further with read-only agents, command rules, and subdirectory instruction overrides where necessary.',
      },
    ],
  },
];
